...
public class CardManagerJDialog extends javax.swing.JDialog implements CardController {
    ...
    public void fillToTable() {
        ...
        for (Card card : cards) {
            model.addRow(new Object[]{
                card.getId(),
                card.getStatus() == 0 ? "Chưa sử dụng" : "Đang sử dụng"
            });
        }
    }
    ...
    @Override
    public void edit() {
        Card entity = cards.get(tblCards.getSelectedRow());
        this.setForm(entity);
        ...
        if (XDialog.confirm("Bạn thực sự muốn xóa các mục chọn?")) {
            for (int i = 0; i < tblCards.getRowCount(); i++) {
                if ((Boolean) tblCards.getValueAt(i, 2)) {
                    dao.deleteById(cards.get(i).getId());
                }
            }
            this.fillToTable();
        }
    ...
    public Card getForm() {
        ...
        if (!idText.isEmpty() && !idText.equals("null")) {
            try {
                entity.setId(Integer.valueOf(idText));
    ...
    public void create() {
        try {
            ...
            // Error already handled in getForm()
        }
    }
